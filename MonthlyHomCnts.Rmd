...
Yearly population estimates and monthly interpolations for all places from the 2010 US census have been prepared and are available at <http://cims.nyu.edu/~tlaetsch/2010_interp_pop.csv>.
...
Now, suppose that we only wanted to inlude homicides of types `FIREARM` and `KNIFE`. This can be acheived by passing `hom_type_col` and `use_hom_types` arguments, which give the name of the column which specifies homicide types and a (list of) the homicide types to include, respectively. 
